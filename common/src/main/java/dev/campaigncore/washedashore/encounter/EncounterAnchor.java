...
public final class EncounterAnchor {
    ...
    public void activate(UUID uuid){status=EncounterStatus.ACTIVE;activeBossUuid=uuid;missingChecks=0;retryAt=0;}
    public void complete(){status=EncounterStatus.COMPLETED;activeBossUuid=null;missingChecks=0;retryAt=0;}
    public void reset(){status=EncounterStatus.DORMANT;activeBossUuid=null;selectedCandidate=null;missingChecks=0;retryAt=0;}
    /** Enters the FAILED cooldown; the encounter becomes retryable after {@link #retryDelayTicks} ticks. */
    public void fail(long now){status=EncounterStatus.FAILED;activeBossUuid=null;missingChecks=0;retryAt=now+retryDelayTicks;}
    /** Enters the ABANDONED cooldown; the encounter becomes retryable after {@link #retryDelayTicks} ticks. */
    public void abandon(long now){status=EncounterStatus.ABANDONED;activeBossUuid=null;missingChecks=0;retryAt=now+retryDelayTicks;}
    ...
    public void restore(EncounterStatus value, UUID boss, long retryAt){status=value;activeBossUuid=boss;this.retryAt=retryAt;}
    public void restore(EncounterStatus value, UUID boss, long retryAt, ResourceLocation candidate){restore(value,boss,retryAt);selectedCandidate=candidate;}
}
